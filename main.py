...
from defenses import Tank, TackShooter, Sniper
...
pygame.display.set_caption("Tower Defense Game 3.0")
...
sniper_icon_rect = pygame.Rect(270, 350, 100, 40)
...
def redraw_game_window():  # draws everything to the screen after their properties are changed
    ...
    pygame.draw.rect(screen, blue, sniper_icon_rect)
    ...
    for sniper in snipers:
        screen.blit(sniper.pic, (sniper.x, sniper.y))

        for bullet in sniper.bullets:
            pygame.draw.rect(screen, red, bullet.rect)

    if picked_tank or picked_sniper:
        if red_bg:
...
while True:
    ...
    for sniper in snipers:
        sniper.move_bullets()
        if hostiles:  # if there are no characters there is nothing to shoot
            if sniper.shoot_count >= sniper.shoot_num:
                sniper.shoot(hostiles[-1])
                sniper.shoot_count = 0
            else:
                sniper.shoot_count += 1
    ...
    if not picked_tank:
        ...
        if picture_rectangle.colliderect(tank_icon_rect) or picture_rectangle.colliderect(tack_icon_rect) or\
                picture_rectangle.colliderect(sniper_icon_rect):
            can_place = False
        ...
        # checks for collisions with the snipers
        if can_place:
            for sniper in snipers:
                if sniper.rect.colliderect(picture_rectangle):
                    can_place = False
                    break
    ...
    if not picked_tack:
        ...
        if picture_rectangle.colliderect(tack_icon_rect) or picture_rectangle.colliderect(tank_icon_rect) or\
                picture_rectangle.colliderect(sniper_icon_rect):
            can_place = False
    ...
    # mechanism to pick up and place a Sniper.
    if not picked_sniper:
        if mouse_rect.colliderect(sniper_icon_rect) and can_click and pygame.mouse.get_pressed(num_buttons=3)[0]:
            picked_tack = False
            picked_tank = False
            picked_sniper = True
    else:
        ...
        if pygame.mouse.get_pressed(num_buttons=3)[0] and can_click:
            if can_place:
                picked_tack = False
                snipers.append(Sniper(mouse_rect.x, mouse_rect.y))
